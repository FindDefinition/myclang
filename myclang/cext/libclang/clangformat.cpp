...
std::unordered_map<std::string, std::string> writeChangedFilesToString(Rewriter& Rewrite) {
  std::unordered_map<std::string, std::string> res;
  for (auto I = Rewrite.buffer_begin(), E = Rewrite.buffer_end(); I != E; ++I) {
    const FileEntry *Entry =
        Rewrite.getSourceMgr().getFileEntryForID(I->first);
    auto FP = Entry->getName().str();
    std::string this_str;
    llvm::raw_string_ostream os(this_str);
    I->second.write(os);
    res[FP] = this_str;
  }
  return res;
...
// Returns true on error.
static bool format(StringRef FileName, std::string inp, std::unordered_map<std::string, std::string>& res) {
  ...
  if (FileName == "-"){
    Code = std::move(MemoryBuffer::getMemBuffer(inp, FileName));
  }else{
  ...
  if (OutputXML || DryRun) {
    ...
    // str = Rewrite.getRewrittenText();
    res = writeChangedFilesToString(Rewrite);
    return true;
  }
  return false;
}
...
int clang_fmt_main(int argc, const char **argv, std::string inp, std::unordered_map<std::string, std::string>& res) {
  ...
  bool Error = false;
  if (FileNames.empty()) {
    Error = clang::format::format("-", inp, res);
    return Error ? 1 : 0;
  }
  if (FileNames.size() != 1 &&
  ...
  for (const auto &FileName : FileNames) {
    ...
    Error |= clang::format::format(FileName, inp, res);
  }
  return Error ? 1 : 0;
}
...
std::tuple<int, std::unordered_map<std::string, std::string>> clangfmt_main(std::vector<std::string> args, std::string inp){
    ...
    std::tuple<int, std::unordered_map<std::string, std::string>> res;
    for (auto &s : args){
        args_ptrs.push_back(s.c_str());
    }
    auto retcode = clang_fmt_main(args_ptrs.size(), args_ptrs.data(), inp, std::get<1>(res));
    std::get<0>(res) = retcode;
    return res;
}
